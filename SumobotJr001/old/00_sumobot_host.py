...
PI_SOKET_TIMEOUT = 0.01             #タイムアウトまでの時間[s]
...
WAIT_TIME = 10  #メインループの待ち時間（ms : これを入れないとウィンドウが）
...
class Ball_detecter_Class:
    ...
    def Get_Blue_Rect_Position_Ratio(self):
        x, y = self.Get_Blue_Rect_Position()
        if x == -1:
            return -200,-200
        else:
...
def main_loop():
    ...
    try:
        ...
        ball_detecter = Ball_detecter_Class(IMAGE_X,IMAGE_X, True, False)
        ...
        try:
            ...
            receiver_sock.settimeout(PI_SOKET_TIMEOUT)          #タイムアウトを0.01sにする
            cmd = ["NoCMD","0","0"]                             #取得コマンドをNoCMDにしておく
        ...
        while main_loop_flag:
            ...
            #青ボールを検出する
            x_ratio, y_ratio = ball_detecter.Get_Blue_Rect_Position_Ratio()
